...
struct player_move{
	...
	struct arma bala[NJUGADORES][MAXAMO]; 
	int balas_disparadas[NJUGADORES];
};
...
void mover_jugador1(char tablero[][N], int pos_jugadores[][COORDENADAS], Direccion apunta[], 
...
void actualizar_estructura(char tablero[][N], int pos_jugadores[][COORDENADAS], Direccion apunta[], 
	arma bala[][MAXAMO], int balas_disparadas[]){
}
...
void bucle_juego(){
    ...
    struct player_move jugador_auxiliar;
	...
	for(int i=0; i<N; i++)
	    for(int j=0; j<N; j++)
		jugador_auxiliar.tablero[i][j] = tablero[i][j];
	...
	do{
	    timeout( 50 );
	    pintar_tablero(tablero, puntuacion);
	    mover_jugador1(tablero, pos_jugadores, apunta, bala, balas_disparadas);
